...
import static io.zeebe.camel.TestConstants.BAR_PAYLOAD;
import static io.zeebe.camel.TestConstants.DUMMY_BPMN;
import static io.zeebe.camel.TestConstants.DUMMY_KEY;
import static io.zeebe.camel.TestConstants.DUMMY_TASK;
import static io.zeebe.camel.TestConstants.FOO_PAYLOAD;
...
public class TaskHandlerTest {
    ...
    private final RouteBuilder createTaskToMock = new RouteBuilder() {
        ...
        public void configure() throws Exception {
            from(TaskUri.topic(zeebe.getDefaultTopic()).lockOwner("test").type(DUMMY_TASK).get())
    ...
    public void a_created_task_is_published_as_json() throws Exception {
        ...
        zeebe.mockEndpoint().expectedMessagesMatches(new JsonBodyPredicate() {
            ...
            public void matches(final JsonNode json) {
                assertThat(json.at("/payloadSource").asText()).as("payload did not match!")
                    .isEqualTo(BAR_PAYLOAD);
    ...
    private final RouteBuilder routesForTaskCompletion = new RouteBuilder() {
        @Override
        public void configure() throws Exception {
            // create job and notify worker
            from(TaskUri.topic(zeebe.getDefaultTopic()).lockOwner("test").type(DUMMY_TASK).get())
                .to("direct:notifyExternalWorker");
            ...
            // complete command is received and the job is completed
            from("direct:completeTask")
                .to(TaskUri.topic(zeebe.getDefaultTopic()).type(DUMMY_TASK).lockOwner("test")
                    .get());
    ...
    public void an_external_worker_can_complete_a_task() throws Exception {
        ...
        zeebe.deploy(DUMMY_BPMN);
        zeebe.startProcess(DUMMY_KEY, FOO_PAYLOAD);
        // route processing completes job, see above
