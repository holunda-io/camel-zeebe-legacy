...
public class EndpointConfigurationTest
{
    ...
    {
        EndpointConfiguration.Remaining remaining = EndpointConfiguration.remaining("topic/task");

        assertThat(remaining.getTopic()).isEqualTo("topic");
        assertThat(remaining.getSubject()).hasValue("task");
        assertThat(remaining.getType()).isEmpty();
    }
    ...
    {
        EndpointConfiguration.Remaining remaining = EndpointConfiguration.remaining("topic/task/type");
    ...
    public void get_universal_handler()
    {
        ...
        assertThat(configuration.getHandler()).isEqualTo(Handler.UNIVERSAL_EVENT);
    }

    private static EndpointConfiguration configuration(DummyUri dummyUri) {
        return new EndpointConfiguration(dummyUri.get(), dummyUri.getBase(), dummyUri.getParameters(), mock(ZeebeComponent.class));
    }
